...
namespace Proyecto_PAVI2021.Datos.Daos
{
    class LoteDao : ILote
    {
        ...
        {
            string consulta = "SELECT l.Id_Lote, l.Fecha_Lote, l.Cantidad_Fabricada, l.Legajo_Empleado, CONCAT_WS(', ', p.Nombre, p.Apellido) NombreCompleto, l.Stock_Lote, m.Nombre, ms.Descripcion, l.Confirmacion_Lote " +
                              "FROM LOTE l " +
                              "JOIN PERSONAL p on p.Legajo = l.Legajo_Empleado " +
                              "JOIN MATERIALES m on l.Id_Material = m.Id_Material " +
                              "JOIN MARCAS ms on ms.Id_Marca = m.Id_Marca " +
                              "WHERE l.Borrado = 0 ";
